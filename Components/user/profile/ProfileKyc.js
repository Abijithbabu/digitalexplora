...
function ProfileKyc({ preloadedValues, getKyc }) {
  ...
  const [kyc, setKyc] = useState({
    ...
    upiId: preloadedValues?.upiId ? preloadedValues?.upiId : "",
    aadhar: {
      title: "aadhar",
  ...
  const uploadToS3 = async (name, file, id) => {
    ...
    return await axios.put(url, file, options).catch((err) => {
      console.log(err);
      setLoading(0);
    });
  };
  ...
  return (
    <div className="lg:w-1/2 mb-10">
      <h3 className="text-gray-800 text-lg font-bold mb-4 text-center">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          disabled={isUpdating}
          className="userBtn w-full ml-auto mt-8 cursor-pointer"
        >
